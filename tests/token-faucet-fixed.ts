...
// 导入代币合约的IDL
const idlContent = fs.readFileSync("./target/idl/token_faucet.json", {encoding: "utf8"});
...
describe("token_faucet", () => {
  ...
  // @ts-ignore - Program构造函数类型错误，实际运行时不影响
  const program = new anchor.Program(idl, provider, programId);
  ...
  it("初始化代币铸造器", async () => {
    try {
      ...
      // @ts-ignore - 运行时account.faucet会存在
